...
<template>
  ...
  <div>
    <h2>{{ home.home.titreSectionTousLesArticles }}</h2>
    <div class="section-AllArticles-articles">
      <div class="section-AllArticles-articles-article" v-for="article in home.home.listesDesArticles" :key="article.id">
        <img :src="article.articleBody[0].image.url" alt="imageArticle" />
        <p>{{ formatDate(article.articleBody[0]._publishedAt)}}</p>
        <div class="article-author">
          <img :src="article.articleBody[0].auteur[0].profileImage.url" alt="imageAuteur" class="roundImageAuthor" />
          <p>{{ article.articleBody[0].auteur[0].nomPrenom }}</p>
        </div>
        <h3>{{ article.articleBody[0].articleTitre }}</h3>
        <div v-if="article.articleBody[0].paragraphe && article.articleBody[0].paragraphe.value">
          <div v-for="(paragraph, index) in article.articleBody[0].paragraphe.value.document.children" :key="index">
            <p v-if="paragraph.type === 'paragraph'">
